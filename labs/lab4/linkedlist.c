...
// TODO
  if (head->v == v) return head->next;

  node *i = head;
  while (i->next != NULL && i->next->v != v) i = i->next;
  
  if (i->next != NULL && i->next->v == v) i->next = i->next->next;
  else error_message(ERR_NOTFOUND);
...
node *reverse_list(node *head) {
  ...
  node *slow = head, *fast = head->next, *curr = head->next;
  head->next = NULL;
