...
void * thread_c(void * arg_in)
{
    ...
    while (arg->result != 0) {
        pthread_mutex_lock(&arg->mutex);
        while (arg->status != S_GUESS) pthread_cond_wait(&arg->cond_guess, &arg->mutex);
...
void * thread_p(void *arg_in)
{
    ...
    pthread_mutex_lock(&arg->mutex);
    while (arg->status != S_MAX) pthread_cond_wait(&arg->cond_result, &arg->mutex);
    max = arg->max;
    pthread_mutex_unlock(&arg->mutex);
    ...
    do { 
        ...
        while (arg->status != S_RESULT) pthread_cond_wait(&arg->cond_result, &arg->mutex);
        result = arg->result;
        pthread_mutex_unlock(&arg->mutex);
...
int main(int argc, char *argv[])
{
    ...
    pthread_create(&threads[0], NULL, (void *)thread_c, &arg);
    pthread_create(&threads[1], NULL, (void *)thread_p, &arg);
