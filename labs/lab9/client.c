...
int main(int argc, char *argv[])
{
    ...
    if (get_number(buf, &max) != 0) die("get_number 1 failed");
    
    do { 
        guess = (min + max) / 2;
        if (send_int(sockfd, guess) != 0) die("send_int failed");

        memset(buf, 0, sizeof(buf));

        if (recv_lines(sockfd, buf, sizeof(buf)) == -1) die("recv_lines failed");
        if (get_number(buf, &result) != 0) die("get_number 2 failed");
        if (result == 1) min = guess + 1;
        else if (result == -1) max = guess - 1;
        sleep(1);
    } while (result != 0);

    recv_lines(sockfd, buf, sizeof(buf));
    printf("%s", buf);
