...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    if (write(fdc[1], &max_value, sizeof(int)) == -1) die("write max_value failed");
    //  repeat the following until guess from parent is correct 
    int result = 2;
    int guess;
    while (result != 0) {
    //      wait for a guess from parent 
        if (read(fdp[0], &guess, sizeof(int)) == -1) die("read guess failed");
    //      call gmn_check()
        result = gmn_check(&gmn, guess);
    //      send the result to parent
        if (write(fdc[1], &result, sizeof(int)) == -1) die("write result failed");
    }
    //  send the final message back (as a string) 
        write(fdc[1], gmn.message, sizeof(gmn.message));
...
int main(int argc, char *argv[])
{
    ...
    if (read(fdc[0], &max, sizeof(int)) == -1) die("read max failed");
    ...
    do { 
        ...
        if (write(fdp[1], &guess, sizeof(int)) == -1) die("write guess failed");
        //     wait for the result from the child
        if (read(fdc[0], &result, sizeof(int)) == -1) die("read result failed");
