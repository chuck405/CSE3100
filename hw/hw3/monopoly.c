...
int one_round(int m, int n, TPlayer p[], TProperty prop[])
{
	for(int i = 0; i < m; i++)
	{
		...
		if (steps + p[i].loc > n-1) {
			p[i].loc = (steps + p[i].loc) % n;
			p[i].balance += n;
		}
		else p[i].loc += steps;
		
		if (prop[p[i].loc].owner_id == -1) prop[p[i].loc].owner_id = p[i].id;
		else if (prop[p[i].loc].owner_id != p[i].id) {
			if (!transaction(&p[i], &p[prop[p[i].loc].owner_id], (p[i].loc)+1)) return 0;
		}
	}
	return 1;
}
