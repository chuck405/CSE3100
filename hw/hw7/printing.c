...
{
    // TODO
    printer_t *args = (printer_t *)arg;
    job_queue_t *jq = args->jq;
    ...
    while (! done) {
        ...
        int job = q_fetch_job(jq, args->id);
...
int main(int argc, char *argv[])
{
    ...
    for (int i=0; i<num_printers; i++) {
        ...
        printers[i].njobs = 0;
        printers[i].thread_id = i;

        if (pthread_create(&threads[i], NULL, printer_main, (void *)&printers[i]) != 0) {
