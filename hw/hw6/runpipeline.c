...
void start_program(Program *programs, int num_programs, int cur) 
{
    ...
    if (pid < 0) die("fork() failed.");
        ...
        if (cur != 0) dup2(programs[cur].fd_in, STDIN_FILENO);
        if (cur != num_programs-1) dup2(programs[cur].fd_out, STDOUT_FILENO);
        for (int i=cur+1; i<num_programs;i++) {
            close(programs[i].fd_in);
            close(programs[i].fd_out);
        }

        if (execvp(programs[cur].argv[0], programs[cur].argv) == -1) die("execvp() failed");
    }
    else {
...
int wait_on_program(Program *prog)
{
    ...
    // TODO
    if (waitpid(prog->pid, &exitStatus, 0) == -1) return -1;
...
void prepare_pipes(Program *programs, int num_programs)
{
    ...
    for (int i=1; i<num_programs; i++) {
        int fd[2];
        if (pipe(fd) < 0) die("pipe() failed.");
        programs[i].fd_in = fd[0];
        programs[i-1].fd_out = fd[1];
    }
}
