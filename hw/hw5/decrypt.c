...
{
    int left = 0, right = 58107;
    while (left <= right) {
        int mid = (left + right) / 2;
        int curr = strcmp(word, words[mid]);
...
{
    for (int i = 0; i < len; i++) decrypted[i] = (encrypted[i] >> shift) ^ key;
    decrypted[len] = '\0';
}
...
int read_encrypted(char *filename, int *encrypted)
{
    ...
    while (fread(&encrypted[bytes_read], sizeof(int), 1, fp) == 1) bytes_read += 1;
    fclose(fp);
    return bytes_read;
}
