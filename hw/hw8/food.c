...
void add_to_buffer(int item, int col, two_d_buffer *p)
{
	...
	while (p->counts[col] >= p->size) pthread_cond_wait(&p->produce_cond, &p->mutex);
...
void remove_from_buffer(int *a, int *b, int *c, two_d_buffer *p)
{
	...
	//fill in code below
	pthread_mutex_lock(&p->mutex);
	while (p->counts[0] == 0 && p->counts[1] == 0 && p->counts[2] == 0) pthread_cond_wait(&p->consume_cond, &p->mutex);
    ...
    *c = p->buf[0][2];
	...
	for (int i=0; i<=2; i++) p->counts[i] -= 1;
...
{   
	...
	while(!done)
	{
		...
		//fill in code below
		pthread_mutex_lock(&p->mutex);
		node *curr = remove_first(&(p->head), &(p->tail));
		pthread_mutex_unlock(&p->mutex);
		
		if (curr != NULL) {
			prepare(curr->v);
			add_to_buffer(curr->v, curr->v, q);
			my_data->total += 1;
		}
		free(curr);
...
int main(int argc, char *argv[])
{
	...
	//TODO
	//fill in code below
	for (t=n_consumer; t<n_consumer + n_producer; t++) total += thread_data_array[t].total;
